// sb-action: !egidebinaire
// sb-subaction-id: ef41316b-95d6-4b8e-8a4f-8609f53d24d1
// sb-group: Boss
// sb-trigger: command !egidebinaire
// Capacité Hexadécimeur — Égide Binaire : +CA pour tout le groupe (config: boss_egide_*). Coûte du mana, consomme le tour.
using System;
using System.IO;
...
public class CPHInline
{
    ...
    private const string CLASSE_REQUISE  = "Hexadécimeur";
    ...
    public bool Execute()
    {
        ...
        if (LireValeur(json, "classe") != CLASSE_REQUISE) { CPH.SendMessage(nomJoueur + ", l'Égide Binaire est réservée aux " + CLASSE_REQUISE + "s."); return true; }

        string cfgG = File.ReadAllText(CONFIG_GLOBAL);
        int cout  = int.Parse(LireValeur(cfgG, "boss_egide_mana"));
        int mana  = int.Parse(LireValeur(json, "manaActuels"));
        if (mana < cout) { CPH.SendMessage(nomJoueur + ", pas assez de mana pour l'Égide Binaire (" + mana + "/" + cout + ")."); return true; }

        int caBonus = int.Parse(LireValeur(cfgG, "boss_egide_ca_bonus"));
        int tours   = int.Parse(LireValeur(cfgG, "boss_egide_tours"))
                    + BonusSousClasse(LireValeur(json, "sousClasse"), "egideBonus");

        json = ModifierValeur(json, "manaActuels", (mana - cout).ToString(), false);
        File.WriteAllText(cheminFichier, json);

        etat = ModifierValeur(etat, "buffCaTours", tours.ToString(), false);

        long maintenant = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        AvancerTour(etat, ordreArr, tourIndex, cfgG, maintenant, "🛡️ " + nomJoueur + " déploie l'Égide Binaire : +" + caBonus + " CA pour tout le groupe !");
        return true;
    }
