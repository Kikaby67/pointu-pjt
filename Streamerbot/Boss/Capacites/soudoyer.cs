// sb-action: !soudoyer
// sb-subaction-id: 1c9103a2-f881-4a73-bda5-f7a82c5a35b2
// sb-group: Boss
// sb-trigger: command !soudoyer
// Capacité Cryptolame — Soudoyer : paie des RAM pour tenter d'acheter le boss (faible chance). Config: boss_soudoyer_*.
// Réussite → le boss épargne le Cryptolame à la prochaine riposte. Échec → le boss ne frappe QUE le Cryptolame.
using System;
using System.IO;
...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (LireValeur(json, "classe") != CLASSE_REQUISE) { CPH.SendMessage(nomJoueur + ", Soudoyer est réservé aux " + CLASSE_REQUISE + "s."); return true; }

        string cfgG = File.ReadAllText(CONFIG_GLOBAL);
        int cout   = int.Parse(LireValeur(cfgG, "boss_soudoyer_ram"));
        int chance = int.Parse(LireValeur(cfgG, "boss_soudoyer_chance"));
        int ram    = int.Parse(LireValeur(json, "ram"));
        if (ram < cout) { CPH.SendMessage(nomJoueur + ", pas assez de RAM pour soudoyer le boss (" + ram + "/" + cout + ")."); return true; }

        json = ModifierValeur(json, "ram", (ram - cout).ToString(), false);
        File.WriteAllText(cheminFichier, json);

        bool reussi = new Random().Next(100) < chance;
        string bossNom = LireValeurString(etat, "bossNom");
        string prefix;
        if (reussi)
        {
            etat = ModifierValeurString(etat, "bribeSafe", pseudoKey);
            prefix = "💰 " + nomJoueur + " glisse " + cout + " RAM à " + bossNom + "... marché conclu : il l'épargnera à la prochaine riposte !";
        }
        else
        {
            etat = ModifierValeurString(etat, "bribeCible", pseudoKey);
            prefix = "💢 " + nomJoueur + " tente de soudoyer " + bossNom + " avec " + cout + " RAM... insulté, il jure de ne frapper QUE lui !";
        }

        long maintenant = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        AvancerTour(etat, ordreArr, tourIndex, cfgG, maintenant, prefix);
        return true;
    }
