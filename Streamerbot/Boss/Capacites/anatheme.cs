// sb-action: !anatheme
// sb-subaction-id: deb769cf-bbaa-4e60-983c-e2f416b2daeb
// sb-group: Boss
// sb-trigger: command !anatheme
// Capacité Firewaller — Anathème : malus d'armure sur le boss (chaque attaque inflige +dégâts). Config: boss_anatheme_*.
using System;
using System.IO;
...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (LireValeur(json, "classe") != CLASSE_REQUISE) { CPH.SendMessage(nomJoueur + ", l'Anathème est réservé aux " + CLASSE_REQUISE + "s."); return true; }

        string cfgG = File.ReadAllText(CONFIG_GLOBAL);
        int cout  = int.Parse(LireValeur(cfgG, "boss_anatheme_mana"));
        ...
        etat = ModifierValeur(etat, "bossCaMalusTours", tours.ToString(), false);

        long maintenant = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        AvancerTour(etat, ordreArr, tourIndex, cfgG, maintenant, "🙏 " + nomJoueur + " prononce un Anathème : l'armure de " + LireValeurString(etat, "bossNom") + " faiblit (+" + degats + " dégâts par attaque, " + tours + " tours) !");
        return true;
    }
