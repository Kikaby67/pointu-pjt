// sb-action: !dansesensuelle
// sb-subaction-id: f92b24c1-42fb-4bd4-87f1-1720f900a698
// sb-group: Boss
// sb-trigger: command !dansesensuelle
// Capacité Algorythmancien — Danse sensuelle : +bonus à tous les jets du groupe (dégâts). Config: boss_danse_*.
using System;
using System.IO;
...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (LireValeur(json, "classe") != CLASSE_REQUISE) { CPH.SendMessage(nomJoueur + ", la Danse sensuelle est réservée aux " + CLASSE_REQUISE + "s."); return true; }

        string cfgG = File.ReadAllText(CONFIG_GLOBAL);
        int cout  = int.Parse(LireValeur(cfgG, "boss_danse_mana"));
        int mana  = int.Parse(LireValeur(json, "manaActuels"));
        if (mana < cout) { CPH.SendMessage(nomJoueur + ", pas assez de mana pour la Danse sensuelle (" + mana + "/" + cout + ")."); return true; }

        int bonus = int.Parse(LireValeur(cfgG, "boss_danse_bonus"));
        int tours = int.Parse(LireValeur(cfgG, "boss_danse_tours"))
                  + BonusSousClasse(LireValeur(json, "sousClasse"), "danseBonus");

        json = ModifierValeur(json, "manaActuels", (mana - cout).ToString(), false);
        File.WriteAllText(cheminFichier, json);

        etat = ModifierValeur(etat, "buffDesTours", tours.ToString(), false);

        long maintenant = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        AvancerTour(etat, ordreArr, tourIndex, cfgG, maintenant, "💃 " + nomJoueur + " se lance dans une danse sensuelle : +" + bonus + " à tous les jets du groupe pendant " + tours + " tours !");
        return true;
    }
