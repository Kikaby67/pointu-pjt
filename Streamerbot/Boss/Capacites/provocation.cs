// sb-action: !provocation
// sb-subaction-id: 96cd4154-f9b1-44fd-9127-024b3d0e22c0
// sb-group: Boss
// sb-trigger: command !provocation
// Capacité Hexadécimeur — Provocation : la prochaine riposte vise le tank seul (groupe épargné), il encaisse réduit. Gratuit.
using System;
using System.IO;
...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (LireValeur(json, "classe") != CLASSE_REQUISE) { CPH.SendMessage(nomJoueur + ", la Provocation est réservée aux " + CLASSE_REQUISE + "s."); return true; }

        string cfgG = File.ReadAllText(CONFIG_GLOBAL);

        // Le tank encaissera seul la prochaine riposte (le groupe est épargné)
        etat = ModifierValeurString(etat, "provocateur", pseudoKey);

        long maintenant = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        AvancerTour(etat, ordreArr, tourIndex, cfgG, maintenant, "😤 " + nomJoueur + " provoque " + LireValeurString(etat, "bossNom") + " : la prochaine riposte le visera lui seul, le groupe est protégé !");
        return true;
    }
