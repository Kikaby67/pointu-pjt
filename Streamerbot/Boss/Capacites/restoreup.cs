// sb-action: !restoreup
// sb-subaction-id: 32157442-9523-4ac1-87ea-0e6a3031f8b4
// sb-group: Boss
// sb-trigger: command !restoreup
// Capacité Firewaller — Restore Up : soigne chaque combattant du groupe (config: boss_restoreup_*). Coûte du mana.
using System;
using System.IO;
...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        if (LireValeur(json, "classe") != CLASSE_REQUISE) { CPH.SendMessage(nomJoueur + ", Restore Up est réservé aux " + CLASSE_REQUISE + "s."); return true; }

        string cfgG = File.ReadAllText(CONFIG_GLOBAL);
        int cout  = int.Parse(LireValeur(cfgG, "boss_restoreup_mana"));
        int mana  = int.Parse(LireValeur(json, "manaActuels"));
        if (mana < cout) { CPH.SendMessage(nomJoueur + ", pas assez de mana pour Restore Up (" + mana + "/" + cout + ")."); return true; }

        int soin = int.Parse(LireValeur(cfgG, "boss_restoreup_pv"))
                 + BonusSousClasse(LireValeur(json, "sousClasse"), "restoreupBonus");

        json = ModifierValeur(json, "manaActuels", (mana - cout).ToString(), false);
        File.WriteAllText(cheminFichier, json);

        // Soigne chaque combattant vivant du groupe (plafonné à pvMax)
        int nbSoignes = 0;
        foreach (string p in ordreArr)
        {
            ...
            int pvMax  = int.Parse(LireValeur(pj, "pvMax"));
            if (pv <= 0) continue;
            int nv = Math.Min(pvMax, pv + soin);
            if (nv != pv)
            {
                pj = ModifierValeur(pj, "pvActuels", nv.ToString(), false);
        ...
        AvancerTour(etat, ordreArr, tourIndex, cfgG, maintenant, "💚 " + nomJoueur + " invoque Restore Up : +" + soin + " PV pour " + nbSoignes + " combattant(s) du groupe !");
        return true;
    }
