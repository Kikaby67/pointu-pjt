// sb-action: !defense
// sb-subaction-id: fd9b8425-5a85-4c08-b4ed-5ca2ea09233c
// sb-group: Boss
// sb-trigger: command !defense
// !defense — en combat de boss : lève sa garde (+CA jusqu'à la riposte de ce round). Consomme le tour.
using System;
using System.IO;
...
public class CPHInline
{
    ...
    public bool Execute()
    {
        ...
        int    caBonus = int.Parse(LireValeur(cfgG, "boss_defense_ca_bonus"));

        // Ajoute le joueur à la liste des défenseurs du round (sans doublon)
        string defenseurs = LireValeurString(etat, "defenseurs");
        if (("," + defenseurs + ",").IndexOf("," + pseudoKey + ",") < 0)
            defenseurs = defenseurs == "" ? pseudoKey : defenseurs + "," + pseudoKey;
        etat = ModifierValeurString(etat, "defenseurs", defenseurs);

        long maintenant = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        AvancerTour(etat, ordreArr, tourIndex, cfgG, maintenant, "🛡️ " + nomJoueur + " lève sa garde (+" + caBonus + " CA jusqu'à la riposte) !");
        return true;
    }
